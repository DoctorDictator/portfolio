  ...
  return (
    <div className="w-1/2 max-lg:w-full max-lg:px-20 max-sm:w-full max-sm:px-5 flex flex-col items-center mt-4 pb-8">
      ...
      {educationData.map((edu: I_Education, idx) => {
        // Check if cgpa is a number and greater than 10, then display as percentage
        const cgpaValue =
          typeof edu.cgpa === "string" ? parseFloat(edu.cgpa) : edu.cgpa;
        const isPercentage = !isNaN(cgpaValue) && cgpaValue > 10;
            ...
            <div className="flex w-full px-5 max-sm:px-0 max-sm:pr-1 py-3">
              ...
              <div className="w-full">
                <div>
                  ...
                  {/* CGPA or Percentage Display */}
                  <h2
                    className={`text-xs max-sm:text-[10px] mt-1 text-gray-500`}
                  >
                    {isPercentage
                      ? `Percentage: ${edu.cgpa}%`
                      : `CGPA: ${edu.cgpa}`}
